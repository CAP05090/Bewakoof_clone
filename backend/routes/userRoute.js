            ...
            if(isStrongPassword(password)){
                bcrypt.hash(password, 5, async(err, hash)=>{
                    if(err){
                        ...
                        let user = new UserModel({username, password:hash, email})
                        await user.save()
        ...
        if(user){
            bcrypt.compare(password, user.password, function(err, result) {
                if(err){
                    ...
                    const Atoken = jwt.sign({userId:user._id, user:user.username}, process.env.AccessKey)
...
// User Refresh to get new access token
/**
 * @swagger
 * /users/refresh:
 *  get:
...
userRouter.get("/refresh", (req, res)=>{
    ...
    if (Rtoken){
        ...
        res.status(404).send({"msg":"Unauthorised User"})
    }
})
